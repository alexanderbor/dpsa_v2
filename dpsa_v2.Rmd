...
#setwd("/Users/szasulja/Dropbox/Anna_Article/")
...
library(dplyr)
library(tidyr)
source("C:/Users/Bor/Dropbox/Anna_Article/summarySE_prez.R")

# Load data
data <- read.table("C:/Users/Bor/Dropbox/Anna_Article/exp1_data.csv", header = T, sep = ",")

#Tidy data 
plotdata <- data %>%
        gather(DV.f, DV, contains("T3"), na.rm = T) %>%
        mutate(DV.f = factor(DV.f, 
                             labels = c("Effective", "Fair", "Personal effect")),
               treat1 = factor(treat1, 
                               labels = c("Democratic", "Expert")))

plotdata <- summarySE(plotdata, measurevar = "DV", groupvars = c("treat1", "DV.f"))
```
...
#Visualize the t.tests for each DV~Techno(Dem) on in a faceted plot

ggplot(plotdata, aes(x = as.factor(treat1), y = mean)) + 
        geom_point() + 
        geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), width=.1, position=position_dodge(0.1)) + xlab("") + ylab("Group means") +
        facet_grid(. ~ DV.f) + 
...
plotdata <- data %>%
  gather(DV.f, DV, c(likecris, fair2, affpay2), na.rm = T) %>%
        mutate(DV.f = factor(DV.f, labels = c("Ineffective", "Fair", "Personal effect")))
```

```{r, echo=FALSE, fig.width=12, fig.height=7}

plotdata <- summarySE(plotdata, measurevar = "DV", groupvars = c("t1f", "DV.f"))

ggplot(plotdata, aes(x = as.factor(t1f), y = mean)) + 
        geom_point() + 
...
```{r, echo=FALSE, fig.width=11, fig.height=6}
# Anti-elitism
data$ant <- rowMeans(data[, 233:238], na.rm = T)
source("C:/Users/Bor/Dropbox/Anna_Article/summarySE_prez.R")
mplot(y = data$fair2, x= data$t1f, f=data$ant ,xlab = "Anti-elitism")
```
...
# Populism 
source("C:/Users/Bor/Dropbox/Anna_Article/summarySE_prez.R")
mplot(y = data$fair2, x= data$t1f, f=data$pop4 ,xlab = "Populism")
```
...
data$crt <- rowSums(data[, 14:16], na.rm = T)
source("C:/Users/Bor/Dropbox/Anna_Article/summarySE_prez.R")
mplot(y = data$fair2, x= data$t1f, f=data$crt, xlab = "Cognitive Reflection Test" )
```
